...
| D018 | Panel width 28HP (up from 20HP) | 41 components (13 params + 8 inputs + 20 outputs) require more width than 20HP provides; 28HP gives comfortable spacing for two individual output columns on the right | 2026-03-13 |
| D019 | Individual outputs on right side in two columns (audio/gate), 10.5mm vertical spacing | Keeps individual per-voice jacks visually grouped and separate from the main polyphonic I/O; 10.5mm spacing is comfortable for patching | 2026-03-13 |
| D020 | Velocity is a direct cable-level passthrough (no engine processing) | Velocity from MIDI-CV doesn't need DSP transformation — users route it to VCAs or other modules downstream; keeps the oscillator focused on oscillator duties | 2026-03-13 |
